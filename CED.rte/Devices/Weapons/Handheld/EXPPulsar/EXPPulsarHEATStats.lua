function Create(self)
	...
	self.HEATParticleUtilityFiringSmokeDataTable.ExploMult = 3.0;
	...
	self.HEATReflectionOutdoorsSound = CreateSoundContainer("Reflection Outdoors CED CED-EXP Pulsar", "CED.rte");
	-- CreateSoundContainer for your indoors tail sound.
	self.HEATReflectionIndoorsSound = CreateSoundContainer("Reflection Indoors CED CED-EXP Pulsar", "CED.rte");
	...
	self.HEATFullMagazineRoundCount = 41;
	...
	self.HEATFakeMagazineMOSRotating = CreateMOSRotating("Fake Magazine MOSRotating CED CED-EXP Pulsar", "CED.rte");
	-- Position to spawn the object at.  Don't include FlipFactor.
	self.HEATFakeMagazineOffset = Vector(3, 3);
	...
	reloadPhase.prepareSound = CreateSoundContainer("Mag Out Prepare CED CED-EXP Pulsar", "CED.rte");
	...
	reloadPhase.afterSound = CreateSoundContainer("Mag Out CED CED-EXP Pulsar", "CED.rte");
	...
	reloadPhase.autoProgressIfFinishedButInterrupted = true;
	reloadPhase.prepareSound = CreateSoundContainer("Mag In Prepare CED CED-EXP Pulsar", "CED.rte");
	reloadPhase.prepareDelay = 950;
	reloadPhase.prepareSoundLength = 950;
	reloadPhase.afterSound = CreateSoundContainer("Mag In CED CED-EXP Pulsar", "CED.rte");
	reloadPhase.afterDelay = 661;
	...
	self.HEATPreSound = CreateSoundContainer("Pre CED CED-EXP Pulsar", "CED.rte");
	-- Delay between being activated and firing.
	self.HEATDelayedFireTimeMS = 60;
end